...
public abstract class TCPSocketManagerBase<T> : Singleton<T> where T : TCPSocketManagerBase<T>
{
    ...
    IEnumerator OnReceiveQueue()
    {
        while (true)
        {
            ...
            try
            {
                ...
                Debug.Log($"Receive Packet : {packet.type.ToString()} | {receiveQueue.Count}");
    ...
    public virtual void Disconnect(bool isReconnect = false)
    {
        StopAllCoroutines();
        isConnected = false;
        ...
        if (isConnected)
        {
            ...
            packet.ConnectGameRequest = new C2S_ConnectGameRequest();
            Send(packet);
